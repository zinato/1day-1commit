...
InfluxDB는 시계열 데이터베이스이다. 다음은 공식 문서가 제공하는 여러 유저 케이스에 대한 최적화 방식에 대한 철학과 장/단점들이다. 

1. 시계열 데이터베이스는, 동일한 데이터를 여러 번 보내면, 고객이 그 여러번 보낸 데이터들을 정확히 동일한 하나의 데이터라고 가정한다.
    * 장접 : 데이터 충돌 문제를 간단하게 해결하여, 쓰기 성능을 향상할 수 있다.
    * 단점 : 중복 데이터를 저장할 수 없으며, 드문 상황에서 데이터를 덮어 쓸 수 있다.
2. 삭제는 드물게 일어난다. 
    * 장점 : 삭제에 대한 접근을 제한하여 쿼리 및 쓰기 성능을 향상할 수 있다.
    * 단점 : 삭제 기능이 제한된다.
3. 수정은 드물게 일어난다. 시계열 데이터는 대부분 수정되지 않는 새로운 데이터이다.
    * 장점 : 수정에 대한 접근을 제한하여 쿼리 및 쓰기 성능을 향상할 수 있다.
    * 단점 : 수정 기능이 제한된다.
4. 대부분의 쓰기는 최근 timestamp와 함께 데이터를 저장하며, 데이터는 시간 오름차순으로 추가된다.
    * 장점 : 시간 오름차순으로 데이터를 추가하는 것이 훨씬 효과이다.
    * 단점 : 무작위 시간 혹은 시간이 오름차순으로 정렬되지 않은 상태에서 포인트를 쓰는 것은 성능이 현저히 떨어진다.
5. 데이터베이스는 대량의 읽기와 쓰기를 처리할 수 있어야 한다.
    * 장점 : 대량의 읽기 및 쓰기를 처리할 수 있다.
    * 단점 : InfluxDB 개발 팀은 성능을 향상시키기 위해서, 몇 가지 트레이드 오프를 강제한다.
6. 데이터를 쓰고 쿼리할 수 있는 것이 강하게 일관된 뷰를 갖는 것보다 중요하다.
    * 장점 : 과부하 환경에서 여러 클라이언트에 의해 데이터베이스를 작성하고, 쿼리할 수 있다.
    * 단점 : 데이터베이스가 과부하 상태일 경우, 쿼리의 결과가 가장 최근의 포인트가 누락될 수 있다.
7. 많은 시계열은 일시적이다. 
    * 장점 : InfluxDB는 이러한 불연속적인 데이터를 잘 관리할 수 있다.
    * 단점 : Schema가 없는 설계는 일부 데이터베이스 기능이 지원되지 않음을 의미한다. `InfluxQL`에서는 테이블 JOIN 기능이 없다. (`Flux`에는 존재)
8. 한 포인트가 결정적이지는 않다. = 스키마가 정해져 있지 않다.
    * 장점 : InfluxDB는 대용량 데이터를 처리하는데 초점을 맞춘다. 보다 데이터 구조에 있어서 유연함을 갖추고 있다.
    * 단점 : 포인트들은 전통적인 의미(기존 SQL은 PRIMARY KEY로 데이터를 구분함)에서 ID를 가지고 있지 않다. (타임스탬프와 시리즈에 의해 구별된다.)


## SQL과의 차이점

`InfluxDB`는 시계열 데이터베이스임에도 `SQL`을 무척 닮아있다. 개인적인 생각으로는 그래서 쿼리 이름도 `InfluxQL`로 지은 것 같다. 뭐 1.8부터는 `Flux`라는 새로운 쿼리 언어를 사용할 수 있으며 2.0 에서는 `InfluxQL`은 deprecated되고 `Flux`로 데이터를 쿼리한다. 근데 `Flux`는 `SQL`과는 사뭇 달라서, 이 문서에서는 다루지 않는다. 일단 `InfluxQL`을 잠깐 살펴보자.

```
SELECT * FROM "my_db"."autogen"."census" WHERE "location"=1 AND time > 2015-08-18T00:06:00Z
```

이런 식으로 쿼리할 수 있다. `SQL`과 정말 많이 닮았다. 또한, 다양한 집계 함수도 제공한다. 무엇보다 시간에 대해서 쿼리할 수 있다.

```
SELECT * FROM "my_db"."autogen"."census" WHERE "location"=1 AND time > now() - 3h
```

이런 식으로 현재 시간 기준으로 3시간 이전 데이터를 쿼리할 수도 있다. 그러나 `InfluxQL`는 단점도 존재한다. 이전 절에서 언급했듯이 `InfluxQL`은 서브쿼리, JOIN 기능이 없다. 그래서 여러 메트릭에 대해 알림을 처리하고 싶을 때가 있다. 이 때, `InfluxDB`와 `InfluxQL`만으로는 해결이 되지 않아서 `Kapacitor`의 `TICKscript`로 데이터를 JOIN을 흉내내곤 한다. 

> Flux에서는 JOIN을 지원합니다. 그러나 형태가 SQL보다는 함수형 프로그래밍에 가깝습니다.
