...
public class AccountAdapter extends User {
    ...
    // 기존에 AccountService 내에 존재하던 authorities 메소드를 이곳으로 옮겨두었습니다.
...
스프링 시큐리티의 유저를 상속받고 우리의 Account를 내부 필드로 가지고 있습니다. 이렇게 해두면, UserDetails 객체로도 쓰일 수 있고 그 하위 필드로 Account를 가지기 때문에 계정 정보를 얻기 쉽습니다. 이제 `AccountService`에서 loadUserByUsername 메소드의 반환 값을 구현한 `AccountAdapter`로 바꾸겠습니다.    
...
이제 계정 정보를 불러올 수 있습니다. `EventController` 내에서는 바로 `Account`를 쓸 수 있도록 CurrentUser 애노테이션을 만들어보겠습니다.

src/main/java/project/accounts/CurrentUser.java
```java
package com.gurumee.keesunrestapi.accounts;
...
위 애노테이션은 메소드 파라미터에 붙일 수 있고 실행시간까지 유지됩니다. `@AuthenticationPrincipal` 애노테이션은 스프링에서 현재 인가된 사용자의 경우 파라미터로 넘겨주는데 사용됩니다. 그리고 expression 값은 만약 인가되지 않은 계정일 땐, null을  인가됬을 때 AccountAdapter의 account를 내보내라는 스프링 표현식입니다. 자 이제 `EventController` 코드를 다음처럼 고쳐주세요.
...
public class EventController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity queryEvent(@PathVariable Integer id, @CurrentUser Account currentUser) {
        ...
        if (event.getManager().equals(currentUser)) {
            resource.add(linkTo(EventController.class).slash(event.getId()).withRel("update-event"));
        }
    ...
    public ResponseEntity updateEvent(@PathVariable Integer id,
        ...
        // 수정된 코드입니다.
        if (!currentUser.getRoles().contains(AccountRole.ADMIN) && !existedEvent.getManager().equals(currentUser)) {
...
createEvents 메소드 내에서는 현재 인가된 유저 정보를 이벤트 내에 저장시켰습니다. 또한 queryEvents의 경우 인가된 사용자라면, create-events 링크를 제공해줍니다. queryEvent의 경우는 인가된 사용자라면 update-event 링크를 제공해주고 updateEvent의 경우 관리자 계정이 아니고, 현재 사용자가 이벤트를 만든 사용자가 아니라면 인가되지 않은 사용자라는 UnAuthorized 결과를 보여주게 만들었습니다.


## 출력 값 제한하기

한데 문제가 생겼습니다. 이 경우 결과 값으로 계정 정보가 모두 노출되기 때문이지요. 이를 고쳐보도록 하겠습니다.

src/main/java/accounts/AccountSerializer.java
```java
package com.gurumee.keesunrestapi.accounts;
...
`ErrorsSerializer`를 구현할 때 처럼 `JsonSerializer`를 상속받은 시리얼라이저 클래스를 만들었습니다. 우리는 계정 정보 중 account_id 만을 사용자에게 보여줄 것입니다. 이�� 이것을 저번처럼 `ObjectMapper`에 등록하는 것이 아닌, `Event`클래스가 JSON화가 될 때만, 계정 정보를 저렇게 JSON화 되게끔 만들것입니다. 그러기 위해선, Event 클래스를 다음처럼 수정해야 합니다.
...
이제 manager 필드가 직렬화될 때 `@JsonSerializer`에 지정된 `AccountSerializer`를 참고하여 직렬화하게 됩니다.


## 깨진 테스트 복구하기

이렇게까지 하면 모두 완성되는데, 이벤트 컨트롤러 테스트 코드가 깨집니다. 이를 고치기 위해선 다음 코드로 수정해야 합니다.
...
public class EventControllerTest extends BaseControllerTest {
    ...
    private Account createAccount() {
        final String email = appProperties.getAdminUsername();
        final String password = appProperties.getAdminPassword();
        final Account account = Account.builder()
                .email(email)
                .password(password)
                .roles(Set.of(AccountRole.USER, AccountRole.ADMIN))
...
자 이렇게 해서 REST API 만들기 수업이 끝났습니다. 모두 고생하셨습니다.
